...
// update land by ward
async function verifyLandByLRO(data:any){
    try{
        console.log("verifylnad", data)
        const response = await api.post("/land/verifyLandByRevenue",data);
        console.log(response);
        return response?.data;
    }catch(error){
        console.log(error)
...
export { registerLand, getLandsByUserId, getLandById, verifyLandByWardOfficer,verifyLandByLRO, verifyOTPandSellProcess, updateLandStatus, getAllLandsOfArea,getLandsWhichIsAvailableForSell }