...
async function getRecommendationForLRO(values){
    try {
        
        const response = await api.post('/recommendation/land', values);
        return response?.data;
    } catch (error) {
        throw error?.response?.data || error;
    }
...
async function verifyRecommendationAndTransferLand(values:any){
    try {
        const response = await api.post('/recommendation/verifyAndTransfer', values);
        return response?.data;
    } catch (error) {
        throw error?.response?.data || error;
    }
}
export { registerRecommendation, getRecommendationForLRO, verifyRecommendationAndTransferLand };